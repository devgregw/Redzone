...
struct CombinedConvectiveRiskProvider: AppIntentTimelineProvider {
    ...
    enum Day: Int, AppEnum {
        ...
        static var caseDisplayRepresentations: [Day: DisplayRepresentation] = [
            .dayOne: "Today",
            .dayTwo: "Tomorrow",
    ...
    struct Configuration: WidgetConfigurationIntent {
        static let title: LocalizedStringResource = "Select Outlook Day"
        static let description = IntentDescription("Select the day to see corresponding outlooks. The wind/hail/tornado breakdown is only supported for today's and tomorrow's outlooks.")
    ...
    private func fetchEntry(configuration: Intent, context: Context) async -> Entry {
        .init(
            ...
            result: await CombinedConvectiveRiskFetcher.fetchCombinedConvectiveRisk(for: configuration.day, includeBreakdown: context.family == .systemMedium)
        )
    }
}
