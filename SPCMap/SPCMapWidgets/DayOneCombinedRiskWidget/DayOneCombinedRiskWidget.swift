//
//  DayOneCombinedRiskWidget.swift
//  SPCMapWidgets
//
...
struct DayOneCombinedRiskWidget: Widget {
    let kind: String = "DayOneCombinedRiskWidget"

    var body: some WidgetConfiguration {
        AppIntentConfiguration(kind: kind, intent: OutlookDayConfigurationIntent.self, provider: Provider()) { entry in
            if #available(iOS 17.0, *) {
                DayOneCombinedRiskWidgetEntryView(entry: entry)
        ...
        .configurationDisplayName("Current Location All Risks")
        .description("Displays the latest day 1 convective categorical, wind, hail, and tornado outlooks at your current location.")
    }
}

extension DayOneCombinedRiskWidget {
                    ...
                    DispatchQueue.main.async {
                        WidgetLocation.shared.requestOneTimeLocation {
                            guard let location = $0 else {
                                continuation.resume(returning: makeTimeline(.error(.noLocation)))
                                return
                            }
                            ...
                            continuation.resume(returning: makeTimeline(.outlook(.now, configuration.day, outlook, (
